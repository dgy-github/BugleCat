...
def test_grpo_advantages_normalises_group():
    adv = rollout.grpo_advantages([1.0, 1.0, 0.0, 0.0])
...
def test_grpo_advantages_all_equal_is_zero():
    assert all(abs(a) < 1e-3 for a in rollout.grpo_advantages([1.0, 1.0, 1.0])), "uninformative group -> 0"
    assert rollout.grpo_advantages([]) == []
...
def test_collect_rollout_runs_tool_loop_then_finishes():
    ...
    orig = rollout.bench_reward
    rollout.bench_reward = lambda task, ws: 1.0   # don't require a real solve
    try:
        ro = rollout.collect_rollout("t1_mathutils", "SYS", chat_fn, tool_exec, max_turns=5)
    finally:
        rollout.bench_reward = orig
...
def test_collect_rollout_unknown_task_errors_gracefully():
    ro = rollout.collect_rollout("no_such_task", "SYS", lambda m: {"content": "x"}, lambda tc, ws: "")
...
def test_collect_group_sizes_and_advantages():
    ...
    original_bench_reward = rollout.bench_reward
    rollout.bench_reward = lambda task, ws: 1.0
    try:
        g = rollout.collect_group("t1_mathutils", "SYS",
                            lambda m: {"content": "done", "tool_calls": []},
                            lambda tc, ws: "", n=4)
    finally:
        rollout.bench_reward = original_bench_reward
