...
def test_export_writes_and_filters():
    ...
    orig = (export._run_and_capture, export.ev.bench.tasks, export._resolve_system_prompt)
    export._run_and_capture = lambda task, gp, to, model, sp="": next(recs)
    export.ev.bench.tasks = lambda: [type("T", (), {"name": "t_a"})(), type("T", (), {"name": "t_b"})()]
    export._resolve_system_prompt = lambda gp: "stub system"
    try:
        summ = export.export(None, ["t_a", "t_b"], out, repeats=1, reward_pass_only=True)
        lines = out.read_text(encoding="utf-8").splitlines()
        assert len(lines) == 1, lines           # only the passing record kept
        rec = json.loads(lines[0])
        assert rec["task"] == "t_a" and rec["reward"] == 1
        assert rec["schema"] == export.SCHEMA and rec["genome_id"] == "baseline"
        assert summ["records"] == 1 and summ["passed"] == 1
    finally:
        export._run_and_capture, export.ev.bench.tasks, export._resolve_system_prompt = orig
