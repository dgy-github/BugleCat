...
//! On recall: score by keyword overlap with the query, tie-break by recency,
...
impl MemoryStore {
    ...
    pub fn new(dir: impl Into<PathBuf>) -> Self {
        MemoryStore { path: dir.into().join("LEARNINGS.md") }
    }
    ...
    pub fn remember(&self, text: &str, tags: &[String], now: u64) -> std::io::Result<bool> {
        ...
        entries.push(MemoryEntry { ts: now, tags: tags.to_vec(), text: text.to_string() });
    ...
    pub fn recall(&self, query: &str, max_entries: usize, max_chars: usize) -> String {
        ...
        let qwords = keywords(query);
        let mut scored: Vec<(i64, &MemoryEntry)> = entries
            .iter()
            .map(|e| {
                let overlap = score(e, &qwords);
    ...
    pub fn consolidate(&self, similarity_threshold: f64) -> std::io::Result<usize> {
        ...
        for e in entries {
            ...
            let dup = kept.iter().any(|(_, kws)| jaccard(kws, &ws) >= similarity_threshold);
            if dup {
                removed += 1;
            } else {
                kept.push((e, ws));
    ...
    fn write_all(&self, entries: &[MemoryEntry]) -> std::io::Result<()> {
        ...
        for e in entries {
            s.push_str(&format!("<!-- ts:{} tags:{} -->\n{}\n\n", e.ts, e.tags.join(","), e.text));
        }
...
/// Keyword overlap of an entry (text + tags) with the query words.
fn score(e: &MemoryEntry, qwords: &[String]) -> i64 {
    if qwords.is_empty() {
        return 0;
    }
    let hay = format!("{} {}", e.text.to_lowercase(), e.tags.join(" ").to_lowercase());
    qwords.iter().filter(|w| hay.contains(w.as_str())).count() as i64
}
...
fn parse_entries(text: &str) -> Vec<MemoryEntry> {
    ...
    let flush = |cur: &mut Option<(u64, Vec<String>)>, body: &mut Vec<String>, out: &mut Vec<MemoryEntry>| {
        if let Some((ts, tags)) = cur.take() {
            let txt = body.join("\n").trim().to_string();
            if !txt.is_empty() {
                out.push(MemoryEntry { ts, tags, text: txt });
            }
        }
        body.clear();
    };
        ...
        if let Some(header) = line.trim().strip_prefix("<!-- ").and_then(|s| s.strip_suffix(" -->")) {
            ...
            for tok in header.split_whitespace() {
                if let Some(v) = tok.strip_prefix("ts:") {
                    ...
                    tags = v.split(',').filter(|t| !t.is_empty()).map(|t| t.to_string()).collect();
                }
            }
            cur = Some((ts, tags));
        } else if cur.is_some() {
...
mod tests {
    ...
    fn remember_then_round_trips() {
        let s = store("rt");
        assert!(s.remember("Use crate-type=lib on the gnu toolchain", &["build".into()], 100).unwrap());
        let es = s.entries();
        assert_eq!(es.len(), 1);
    ...
    fn recall_scores_by_keyword_overlap() {
        let s = store("recall");
        s.remember("Windows GNU linker overflows on cdylib", &["build".into()], 1).unwrap();
        s.remember("The storyboard panel renders thumbnails", &["gui".into()], 2).unwrap();
    ...
    fn consolidate_is_idempotent() {
        ...
        s.remember("alpha beta gamma delta epsilon", &[], 2).unwrap();
    ...
    fn recall_respects_entry_cap() {
        let s = store("reccap");
        for i in 0..10 {
            s.remember(&format!("note {i} about widgets"), &[], i).unwrap();
        }
